...
const SingleSelect = ({options}) => {
  ...
  const keyboardNavigation = (e) => {
    if (e.key === "ArrowDown") {
      ...
      if (cursor !== -1) setOptions(options[cursor][0])
    };

    if (e.key === "ArrowUp") {
      if (cursor > 0) setCursor(cursor - 1)
      setOptions(options[cursor][0])
    };
    ...
    >
      ...
      {isVisible &&
        ...
        {options
          ...
          .filter((value, i) => ((((value[0]).slice(0,search.length)).toLowerCase()).indexOf(search.toLowerCase()) !== -1 ))
          .map((item, i) => {
          return (
            <div
            className="dropdown-value"
            key={i}
            onClick={() => setOptions(item[0])}>
            <span>{item[0]}</span>
            <img src={item[1]} alt="logo"/>
            </div>
            // Uncomment to render dropdown menu without logos
